...
public interface SmilePatientRepository extends Neo4jRepository<SmilePatient, Long> {
    ...
    @Query("""
           OPTIONAL MATCH (s: Sample {smileSampleId: $smileSampleId})<-[hs:HAS_SAMPLE]-(p: Patient)
           <-[ia:IS_ALIAS]-(pa: PatientAlias)
    ...
    @Query("""
           MATCH (p: Patient)<-[:IS_ALIAS]-(pa: PatientAlias {value: $oldCmoId, namespace: 'cmoId'})
    ...
    @Query("""
           MATCH (p: Patient {smilePatientId: $patient.smilePatientId})<-[:IS_ALIAS]-(pa: PatientAlias)
           DETACH DELETE p, pa
           """)
